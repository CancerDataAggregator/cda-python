...
def test_demonstrate_list_to_df_with_search_replacement():
    ...
    df = json_normalize(
        data=unique_terms(
            col_name="primary_diagnosis_site", show_counts=True, host=host
        ).to_list()
    )
    new_way = df.loc[
    ...
    old_way = unique_terms(
        "primary_diagnosis_site", show_counts=True, host=host
    ).to_dataframe(search_fields="primary_diagnosis_site", search_value="gland")
